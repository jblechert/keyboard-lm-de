...
def get_llama_bench(abi, force=False):
    ...
    # Asset name pattern: llama-b<N>-bin-android-<abi>.zip
    asset = next(
        (a for a in release["assets"]
         if f"android-{abi}" in a["name"] and a["name"].endswith(".zip")),
        None
    )
    if not asset:
        names = [a["name"] for a in release["assets"]]
        print(f"  Kein Android-{abi} Asset in {tag}. Assets: {names}", file=sys.stderr)
        ...
        with zipfile.ZipFile(tmp.name) as zf:
            bench_entry = next(
                (n for n in zf.namelist()
                 if Path(n).name == BENCH_BINARY or Path(n).name == f"{BENCH_BINARY}-android"),
                None
            )
            if not bench_entry:
                # try any file named llama-bench*
                bench_entry = next((n for n in zf.namelist() if "llama-bench" in n and "/" not in n.lstrip("/")), None)
            if not bench_entry:
...
def parse_bench_output(stdout):
    ...
    Returns list of dicts with keys: test, t_s
    """
    results = []
    for line in stdout.splitlines():
        ...
        # Look for rows with "pp NNN" or "tg NNN"
        for i, p in enumerate(parts):
            m = re.match(r"^(pp|tg)\s+(\d+)$", p)
            if m:
                # t/s column is last non-empty
                ts_str = parts[-1] if parts else ""
                ts_m = re.search(r"([\d.]+)\s*(?:±|\+-)?", ts_str)
                if ts_m:
                    results.append({
                        "test": p,
                        "kind": m.group(1),
...
def report(results, model_name, device_model, android_ver, pp):
    ...
    if pp_ts and tg_ts:
        ...
        total_ms = pp_ms + tg_ms

        print(f"\n── Keyboard-Latenz ({pp} Tokens Kontext) {'─'*20}")
        print(f"  Kontext verarbeiten:  {pp_ms:6.1f} ms")
        print(f"  1 Token generieren:   {tg_ms:6.1f} ms")
        print(f"  Erste Vorhersage:     {total_ms:6.1f} ms  <- keyboard feel")
        print(f"  Gboard-Ziel:           <20.0 ms")

        if total_ms < 10:
            rating = "sehr gut  -- problemlos fuer 72M-Upscale"
        elif total_ms < 20:
            rating = "gut       -- Gboard-Level"
        elif total_ms < 40:
            rating = "akzeptabel -- Flagship-only fuer 72M"
        else:
            rating = "zu langsam -- 36M schon grenzwertig"
