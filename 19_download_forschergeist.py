#!/usr/bin/env python3
"""
Lädt VTT-Transkripte von Forschergeist herunter und extrahiert saubere deutsche Sätze.

Quelle:  Forschergeist von Tim Pritlove (Metaebene Personal Media)
         https://forschergeist.de
Lizenz:  CC BY-NC-SA 3.0 DE
         https://creativecommons.org/licenses/by-nc-sa/3.0/de/
         Namensnennung: Tim Pritlove, forschergeist.de

URL-Muster (aus RSS-Feed):
  https://forschergeist.de/podcast/fgNNN-slug/?podlove_transcript=webvtt

Output: data/forschergeist_de.txt

Usage:
  .venv_ml/bin/python 19_download_forschergeist.py [--dry-run]
"""
...
OUT_FILE = Path("data/forschergeist_de.txt")
...
def parse_rss(feed: str) -> list[tuple[str, str]]:
    """Gibt Liste von (titel, episode_url) zurück."""
    items = re.findall(r"<item>(.*?)</item>", feed, re.DOTALL)
    result = []
    for item in items:
        title = re.search(r"<title><!\[CDATA\[(.*?)\]\]>|<title>(.*?)</title>", item)
        link  = re.search(r"<link>(https://forschergeist\.de/podcast/[^<]+)</link>", item)
        if not link:
            continue
        t = (title.group(1) or title.group(2) or "?").strip() if title else "?"
        result.append((t, link.group(1).rstrip("/") + "/?podlove_transcript=webvtt"))
    return result
